...
- [ ] 14.2 增强视频 readyState 检查
  - 在创建视频纹理前检查 video.readyState >= HAVE_CURRENT_DATA
  - 监听多个视频事件（playing, canplay, loadeddata）

  - 在所有事件处理中都检查 readyState
  - 添加超时机制：5 秒内未准备好则报错
  - _需求: 8.1, 8.6_
...
- [ ] 14.3 确保视频纹理正确更新
  ...
  - 显式设置 videoTexture.needsUpdate = true
  - 验证视频元素的 currentTime 是否在变化
  - 添加调试日志记录纹理更新状态
  - _需求: 8.2_



- [ ] 14.4 优化 panorama:loaded 事件触发
  - 确保事件在视频纹理创建且首帧渲染后触发
  - 添加视觉验证：首帧渲染成功后才触发
  - 在 LoadingScreen 组件中正确处理该事件
  - _需求: 8.4_

- [ ] 14.5 添加占位内容
  - 在视频纹理创建前显示占位颜色或图片
  - 使用 MeshBasicMaterial 的 color 属性设置占位色
  - 视频纹理准备好后替换占位内容
  - _需求: 8.5_
...
- [ ]\* 16. 编写文档
  ...
  - 添加部署注意事项
  - 添加已知问题和解决方案（百度浏览器等）
  - _需求: 1.1, 1.2, 1.3, 1.4, 2.1, 8.1, 8.2, 8.3_
